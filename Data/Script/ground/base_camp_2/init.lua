	...
	while state > -1 do
		...
		elseif state == 2 then
			...
			if total > GAME:GetPlayerMoney() then
				...
				if #cart == 1 then
					local name = catalog[cart[1]].Item:GetName()
		...
		elseif state == 4 then
			...
			if #cart == 1 then
				...
				msg = STRINGS:Format(MapStrings['Shop_Sell_One'], total, item:GetName())
			else
		...
		elseif state == 2 then
			...
			if total > GAME:GetPlayerMoney() then
				...
				if #cart == 1 then
					...
					msg = STRINGS:Format(MapStrings['Appraisal_Choose_One'], total, item:GetName())
				else
				...
				if result then
					...
					GROUND:MoveInDirection(chara, Direction.Up, 18)
					...
					GROUND:MoveInDirection(chara, Direction.Down, 18)
	...
	while state > -1 do
		...
		elseif state == 2 then
			...
			if free_slots > 0 then
				UI:WaitShowDialogue(STRINGS:Format(MapStrings['Swap_Give_Choose'], receive_item:GetName()))
		...
		elseif state == 3 then
			...
			for ii = 1, #tribute, 1 do
				...
				table.insert(give_items, give_item:GetName())
			end
			
			local itemEntry = RogueEssence.Data.DataManager.Instance:GetItem(trade.Item)
			local total = Prices[itemEntry.Rarity]
			
			UI:WaitShowDialogue(STRINGS:Format(MapStrings['Swap_Confirm_001'], STRINGS:CreateList(give_items), receive_item:GetName()))
			...
			if result then
				...
				UI:ResetSpeaker()
				SOUND:PlayFanfare("Fanfare/Treasure")
				UI:WaitShowDialogue(STRINGS:Format(MapStrings['Swap_Give'], player:GetDisplayName(), receive_item:GetName()))
				...
				UI:WaitShowDialogue(STRINGS:Format(MapStrings['Item_Give_Storage'], receive_item:GetName()))
	...
	while state > -1 do
		...
		elseif state == 3 then
			...
			if GAME:CanLearn(member) then
				...
				UI:WaitShowDialogue(STRINGS:Format(MapStrings['Tutor_Remember_Success'], member.BaseName, moveEntry.Name:ToLocal()))
				state = 0
			else
