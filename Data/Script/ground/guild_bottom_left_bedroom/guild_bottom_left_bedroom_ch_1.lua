...
function guild_bottom_left_bedroom_ch_1.Breloom_Action(chara, activator)
	...
	if not SV.Chapter1.MetBreloomGirafarig then 
		...
		UI:WaitShowDialogue(tail:GetDisplayName() .. " won't stop calling me names no matter how many times I ask him not to!")
		GAME:WaitFrames(20)
		...
		UI:SetSpeaker(STRINGS:Format("\\uE040"), true, breloom.CurrentForm.Species, breloom.CurrentForm.Form, breloom.CurrentForm.Skin, breloom.CurrentForm.Gender)
		UI:SetSpeakerEmotion("Normal")
		UI:WaitShowDialogue("Usually when a pair of Pokémon we don't know show up in our room,[pause=10] it's new rookies looking around the place.")
		GAME:WaitFrames(20)
		...
		UI:WaitShowDialogue("(I don't know if you can tell,[pause=10] but " .. girafarig:GetDisplayName() .. " is a bit on the odd side.")
		UI:WaitShowDialogue("(I mean I love the guy but,[pause=10] he's definitely a little...[pause=10] off.)")
		UI:WaitShowDialogue("(Well, [pause=10] he thinks his tail can talk.[pause=0] He gave it a name and everything.)")
